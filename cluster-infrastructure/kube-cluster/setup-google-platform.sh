...
if [[ -f "service-account.json" ]]; then
    echo "${red}It turns out you have already service-account.json and now it moved  to old-service-account.json${reset}"
    /bin/mv service-account.json old-service-account.json
    gcloud iam service-accounts keys create service-account.json --iam-account="$SERVICE_ACCOUNT_EMAIL"
    echo "${green}The <service-account.json> is created!!${reset}"
else
    gcloud iam service-accounts keys create service-account.json --iam-account="$SERVICE_ACCOUNT_EMAIL"
    echo "${green}The <service-account.json> is created!!${reset}"
fi
